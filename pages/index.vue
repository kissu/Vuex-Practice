<template>
  <div>
    ...
    <form action="submit" @submit.prevent="sendCityName()">
      ...
      <input type="text" id="query" v-model="cityName" placeholder="Search for a city!">
      <button type="submit">Submit</button>
    </form>
    <h3>{{ lat }}</h3>
    <WeatherApp :someData="geoData" :someCity="cityId" />
  </div>
</template>
...
export default {
  ...
  methods: {
    async sendCityName() {
      await this.$store.dispatch('store/fetchCity', this.cityName)
    }
    // ...mapMutations({
    //   submitCity: ""
    // })
